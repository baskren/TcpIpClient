...
namespace Aptiv.DBCFiles
{
    ...
    public class DBCFile : IDBCFile
    {
        ...
        */
        ...
        public static DBCFile Load(Stream stream)
        {
            if (stream is null)
        ...
        public static DBCFile Load(string filename)
        {
            if (string.IsNullOrWhiteSpace(filename))
                throw new ArgumentException($"filename is null, empty or whitespace");
        ...
        public static DBCFile Load(StreamReader reader)
        {
            if (reader is null)
                throw new ArgumentException($"reader is null");

            var s = reader.ReadToEnd();
            var result = new DBCFile();
            result.Parse(s);
            return result;
        }

        public static async Task<DBCFile> LoadAsync(Stream stream)
        {
            if (stream is null)
                throw new ArgumentException($"stream is null");

            using (StreamReader sr = new StreamReader(stream))
                return await LoadAsync(sr);
        }


        public static async Task<DBCFile> LoadAsync(string filename)
        {
            if (string.IsNullOrWhiteSpace(filename))
                throw new ArgumentException($"filename is null, empty or whitespace");
        ...
        public static async Task<DBCFile> LoadAsync(StreamReader reader)
        {
            if (reader is null)
                throw new ArgumentException($"reader is null");
